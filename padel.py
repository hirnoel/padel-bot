...
st.markdown("""
<style>
    .calendar-wrapper { display: flex; flex-direction: row; height: 720px; font-family: 'Segoe UI', sans-serif; background-color: white; border: 1px solid #e0e0e0; overflow: hidden; }
    .time-col { width: 50px; border-right: 1px solid #eee; background-color: #fafafa; position: relative; flex-shrink: 0; }
    ...
    .court-col { flex: 1; position: relative; border-right: 1px solid #f0f0f0; }
...
if 'is_logged_in' not in st.session_state:
    ...
    try:
        ...
        pass # If it fails, we just stay logged out
...
def render_html_calendar(all_courts_data):
    OPENING_HOUR, CLOSING_HOUR = 8, 23
    PIXELS_PER_HOUR = 48
    css = """<style>.calendar-wrapper{display:flex;flex-direction:row;height:720px;font-family:'Segoe UI',sans-serif;background-color:white;border:1px solid #e0e0e0;overflow:hidden;}.time-col{width:50px;border-right:1px solid #eee;background-color:#fafafa;position:relative;flex-shrink:0;}.time-label{position:absolute;width:100%;text-align:right;padding-right:6px;font-size:11px;color:#999;transform:translateY(-50%);}.court-col{flex:1;position:relative;border-right:1px solid #f0f0f0;}.grid-line{position:absolute;width:100%;border-top:1px solid #f5f5f5;z-index:0;}.grid-line-major{border-color:#eee;}.event-card{position:absolute;width:94%;left:3%;border-radius:4px;font-size:11px;font-weight:600;display:flex;align-items:center;justify-content:center;box-sizing:border-box;z-index:10;white-space:nowrap;overflow:hidden;box-shadow:0 1px 2px rgba(0,0,0,0.05);transition:0.1s;}.event-card:hover{z-index:20;transform:scale(1.02);}.free{background-color:#d1fae5;color:#065f46;border:1px solid #6ee7b7;cursor:pointer;}</style>"""
    html = f'{css}<div class="calendar-wrapper"><div class="time-col">'
    for h in range(OPENING_HOUR, CLOSING_HOUR + 1):
        top = (h - OPENING_HOUR) * PIXELS_PER_HOUR
    ...
    for court_data in all_courts_data:
        ...
        for slot in court_data:
            ...
            top_px = (start_min / 60) * PIXELS_PER_HOUR
